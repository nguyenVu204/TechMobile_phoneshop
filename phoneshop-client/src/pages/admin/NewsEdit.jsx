...
export default function NewsEdit() {
  ...
  // --- FETCH DỮ LIỆU CŨ VÀ DANH MỤC ---
  useEffect(() => {
    const fetchData = async () => {
      try {
        // 1. Lấy danh sách chuyên mục
        const catRes = await axiosClient.get("/newscategories");
        setCategories(catRes.data);

        // 2. Lấy dữ liệu bài viết đang sửa
        ...
        setNewsData({
            ...
            relatedProductId: res.data.relatedProductId || ""
        });
        setLoading(false);
      } catch (error) {
    ...
    <div className="max-w-5xl mx-auto pb-20">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center space-x-4">
          ...
          <h1 className="text-2xl font-bold text-gray-800">Sửa bài viết <span className="text-blue-600">#{id}</span></h1>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 space-y-5">
                ...
                <div>
                    ...
                    <input 
                        ...
                        placeholder="danh-gia-chi-tiet-iphone-15..."
                    />
                </div>

                <div>
                    <label className="block text-sm font-bold text-gray-700 mb-1">Tóm tắt (Trích dẫn hiển thị ngoài trang chủ)</label>
                    <textarea 
                        rows="3" 
                        ...
                        placeholder="Một đoạn ngắn khoảng 2-3 câu giới thiệu bài viết..."
                    />
                </div>
        ...
        <div className="lg:col-span-1 space-y-6">
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 space-y-4">
                <div>
                    ...
                    <select 
                        ...
                        className="w-full border border-gray-200 p-3 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 font-bold text-gray-700 transition cursor-pointer"
                    >
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                    ...
                    {newsData.thumbnail ? (
                        <>
                            ...
                            <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition flex items-center justify-center">
                                <button 
                                    onClick={() => setNewsData({...newsData, thumbnail: ""})}
                                    className="bg-red-500 text-white px-3 py-1.5 rounded-lg text-sm font-bold hover:bg-red-600"
                                >
                                    Xóa ảnh
                                </button>
                            </div>
                ...
                <label className={`w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-blue-50 text-blue-600 rounded-xl border border-blue-100 cursor-pointer hover:bg-blue-100 transition font-bold text-sm ${uploading ? 'opacity-50 pointer-events-none' : ''}`}>
                    {uploading ? <Loader2 size={18} className="animate-spin" /> : <UploadCloud size={18} />}
                    {uploading ? "Đang tải lên..." : "Đổi ảnh khác"}
                    <input type="file" className="hidden" accept="image/*" onChange={handleImageUpload} disabled={uploading}/>
                </label>
            </div>
            
            {/* Gắn link Sản phẩm liên quan */}
            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                <label className="block text-sm font-bold text-gray-700 mb-1">Gắn sản phẩm liên quan</label>
                <p className="text-xs text-gray-500 mb-3">Nhập ID sản phẩm để hiển thị block mua hàng dưới bài viết.</p>
                <input 
                    type="number" 
                    value={newsData.relatedProductId} 
                    onChange={(e) => setNewsData({...newsData, relatedProductId: e.target.value})} 
                    className="w-full border border-gray-200 p-3 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition"
